...
import { sendTrainingZoomUpdate } from "@/lib/training-emails";
...
// One-shot broadcast: notify every registration for the active event
// that the Zoom URL changed. Idempotent via training_email_log kind="zoom_update".
export async function POST(req: Request) {
  ...
  if (!event) {
    return NextResponse.json({ error: "No active training event." }, { status: 400 });
  }
  ...
  for (const reg of regs) {
    ...
    if (sentSet.has(reg.id as string)) { skipped++; continue; }

    try {
      await sendTrainingZoomUpdate(reg.email, reg.full_name ?? "", event);
